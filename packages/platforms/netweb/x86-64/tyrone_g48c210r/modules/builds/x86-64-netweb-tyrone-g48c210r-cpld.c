...
struct tyrone_g48c210r_cpld_data {
	enum cpld_type type;
...
static const struct i2c_device_id tyrone_g48c210r_cpld_id[] = {
	{ "g48c210r_cpld", tyrone_g48c210r_cpld },
	{ }
};
MODULE_DEVICE_TABLE(i2c, tyrone_g48c210r_cpld_id);
...
enum tyrone_g48c210r_cpld_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
			char *buf);
static int tyrone_g48c210r_cpld_read_internal(struct i2c_client *client,
			u8 reg);
static int tyrone_g48c210r_cpld_write_internal(struct i2c_client *client,
			u8 reg, u8 value);

/*fan sysfs*/
static struct tyrone_g48c210r_cpld_data *tyrone_g48c210r_fan_update_device(
    						struct device *dev);
...
static struct attribute *tyrone_g48c210r_cpld_attributes[] = {
...
static const struct attribute_group tyrone_g48c210r_cpld_group = {
	.attrs = tyrone_g48c210r_cpld_attributes,
};
			...
			char *buf)
{
	...
	struct tyrone_g48c210r_cpld_data *data = i2c_get_clientdata(client);

	mutex_lock(&data->update_lock);

	for (i = 0; i < size[data->type]; i++) {
		status = tyrone_g48c210r_cpld_read_internal(client, regs[data->type][i]);
		if (status < 0) {
			goto exit;
		}
			 ...
			 char *buf)
{
	...
	mutex_lock(&data->update_lock);
	status = tyrone_g48c210r_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
	...
	status = tyrone_g48c210r_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0))
		goto exit;
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = tyrone_g48c210r_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0))
		goto exit;
...
static void tyrone_g48c210r_cpld_add_client(struct i2c_client *client)
{
...
static void tyrone_g48c210r_cpld_remove_client(struct i2c_client *client)
{
							  ...
							  const char *buf, size_t count)
{
	...
	tyrone_g48c210r_cpld_write_internal(client, fan_reg[1], reg_val);
	tyrone_g48c210r_cpld_write_internal(client, fan_reg[2], reg_val);
	data->valid = 0;
...
static u8 is_fan_fault(struct tyrone_g48c210r_cpld_data *data, enum fan_id id)
{
	u8 ret = 1;
							  ...
							  char *buf)
{
	...
	data = tyrone_g48c210r_fan_update_device(dev);
	if (!data->valid) {
		ret = -EIO;
		goto exit;
	}
					...
					struct device *dev)
{
	...
	if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
		...
		dev_dbg(&client->dev, "Starting tyrone_g48c210r_fan update\n");
		data->valid = 0;

		/* Update fan data
		 */
		for (i = 0; i < ARRAY_SIZE(data->reg_fan_val); i++) {
			int status = tyrone_g48c210r_cpld_read_internal(client, fan_reg[i]);
			if (status < 0) {
				data->valid = 0;
 ...
 */
static int tyrone_g48c210r_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct tyrone_g48c210r_cpld_data *data;
	int ret = -ENODEV;

	if (!i2c_check_functionality(adap, I2C_FUNC_SMBUS_BYTE))
		goto exit;

	data = kzalloc(sizeof(struct tyrone_g48c210r_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
	...
	ret = sysfs_create_group(&client->dev.kobj, &tyrone_g48c210r_cpld_group);
	if (ret)
		goto exit_free;

	tyrone_g48c210r_cpld_add_client(client);
	return 0;
...
static int tyrone_g48c210r_cpld_remove(struct i2c_client *client)
{
	struct tyrone_g48c210r_cpld_data *data = i2c_get_clientdata(client);

	tyrone_g48c210r_cpld_remove_client(client);

	/* Remove sysfs hooks */
	sysfs_remove_group(&client->dev.kobj, &tyrone_g48c210r_cpld_group);
...
static int tyrone_g48c210r_cpld_read_internal(struct i2c_client *client, u8 reg)
{
...
static int tyrone_g48c210r_cpld_write_internal(struct i2c_client *client, u8 reg,
										u8 value)
{
	int status = 0, retry = I2C_RW_RETRY_COUNT;
...
int tyrone_g48c210r_cpld_read(unsigned short cpld_addr, u8 reg)
{
	struct list_head   *list_node = NULL;
	struct cpld_client_node *cpld_node = NULL;
	int ret = -EPERM;
	...
	list_for_each(list_node, &cpld_client_list) {
		...
		if (cpld_node->client->addr == cpld_addr) {
			ret = tyrone_g48c210r_cpld_read_internal(cpld_node->client, reg);
			break;
		}
	}
...
EXPORT_SYMBOL(tyrone_g48c210r_cpld_read);

int tyrone_g48c210r_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
	struct list_head   *list_node = NULL;
	struct cpld_client_node *cpld_node = NULL;
	int ret = -EIO;
	...
	list_for_each(list_node, &cpld_client_list) {
		...
		if (cpld_node->client->addr == cpld_addr) {
			ret = tyrone_g48c210r_cpld_write_internal(cpld_node->client,reg, value);
			break;
		}
	}
...
EXPORT_SYMBOL(tyrone_g48c210r_cpld_write);

static struct i2c_driver tyrone_g48c210r_cpld_driver = {
	.driver = {
		.name = "tyrone_g48c210r_cpld",
		.owner= THIS_MODULE,
	},
	.probe = tyrone_g48c210r_cpld_probe,
	.remove = tyrone_g48c210r_cpld_remove,
	.id_table = tyrone_g48c210r_cpld_id,
};

static int __init tyrone_g48c210r_cpld_init(void)
{
	mutex_init(&list_lock);
	return i2c_add_driver(&tyrone_g48c210r_cpld_driver);
}

static void __exit tyrone_g48c210r_cpld_exit(void)
{
...
module_init(tyrone_g48c210r_cpld_init);
module_exit(tyrone_g48c210r_cpld_exit);
